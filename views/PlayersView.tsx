        ...
        <div className="flex flex-col h-full relative">
            <div className="p-4 bg-slate-800/50 border-b border-slate-700 flex justify-between items-center shadow-md z-10">
                <div>
                    ...
                    <p className="text-xs text-slate-400">
                        {players.filter(p => p.isConnected).length} Online / {players.length} Total
                    </p>
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar pb-32">
                {sortedPlayers.map((p) => (
                    <div
                        key={p.uuid}
                        className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${p.isConnected
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-1.5 text-xs">
                                <div className={`w-2 h-2 rounded-full ${p.isConnected ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-slate-600'}`} />
                                <span className={p.isConnected ? 'text-slate-400' : 'text-slate-600'}>
