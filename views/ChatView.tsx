          ...
          <div className="bg-slate-800 border border-orange-500/50 rounded-xl p-4 shadow-2xl">
            ...
            {!hasVoted && activeVote.requesterId !== myId && (
              <div className="flex gap-2 mt-2">
                <button
                  onClick={() => onVote(true)}
                  className="flex-1 bg-green-500/20 hover:bg-green-500/30 text-green-400 py-2 rounded-lg font-bold transition-colors"
                >
                  Approve
                </button>
              </div>
        ...
        {gameState.messages.map((msg: any) => {
          if (msg.isSystem) {
            return (
              <div
                key={msg.id}
                className="text-center text-xs text-slate-500 my-2 italic px-8"
              >
                {msg.content}
              </div>
            );
