...
const Toast: React.FC<{ notification: AppNotification; onDismiss: (id: string) => void }> = ({ notification, onDismiss }) => {
    ...
    const getStyles = () => {
        switch (notification.type) {
            case 'success': return 'bg-slate-800 border-green-500/50 shadow-green-500/10';
            case 'warning': return 'bg-slate-800 border-yellow-500/50 shadow-yellow-500/10';
            case 'error': return 'bg-slate-800 border-red-500/50 shadow-red-500/10';
            default: return 'bg-slate-800 border-cyan-500/50 shadow-cyan-500/10';
        }
    };
