...
import { GameState, ConnectionStatus, P2PMessage } from "../../types";
...
export const usePeerCoordination = ({
  ...
  const servicePeerUpdateTimerRef = useRef<ReturnType<
    typeof setInterval
  > | null>(null);
  ...
  const electNewMod = useCallback(() => {
    const candidates = gameStateRef.current.players
      .filter(
        (p) =>
          p.id !== modPeerIdRef.current &&
          (p.id === myId || connectionsRef.current.has(p.id)),
      )
  ...
  const updateServicePeers = useCallback(() => {
    const onlinePlayers = gameStateRef.current.players.filter(
      (p) => p.isConnected,
    );
    const scoredPlayers = onlinePlayers
      .map((p) => {
    ...
    const newServicePeers = mod
      ? [mod.id, ...bestPeers.map((p) => p.id)]
      : bestPeers.slice(0, 1).map((p) => p.id);
  ...
  useEffect(() => {
    ...
    const tickHeartbeat = () => {
      if (status === ConnectionStatus.CONNECTED) {
        ...
        broadcast({
          type: "HEARTBEAT",
          payload: { id: myId, timestamp: now, sequence },
        });
        ...
        connectionsRef.current.forEach((_, peerId) => {
          ...
          if (lostCount > 0) {
            metrics.packetLoss = Math.min(
              1,
              metrics.packetLoss * 0.8 + (lostCount / 5) * 0.2,
            );
          } else {
        ...
        const avgJitter =
          metricsList.length > 0
            ? metricsList.reduce<number>(
                (acc, m: any) => acc + (m.jitter || 0),
                0,
              ) / metricsList.length
            : 0;
        ...
        if (avgJitter < 5) {
          currentHeartbeatInterval = Math.min(
            ...
            NETWORK_CONFIG.ADAPTIVE_HEARTBEAT_MAX_MS,
          );
        } else {
    ...
    const monitorTimer = setInterval(() => {
      if (
        ...
        status !== ConnectionStatus.MIGRATING
      )
        return;
      const iAmMod = gameStateRef.current.players.find(
        (p) => p.id === myId,
      )?.isMod;
      if (iAmMod) return;
      ...
      const timeout =
        status === ConnectionStatus.MIGRATING
          ? 10000
          : NETWORK_CONFIG.HOST_TIMEOUT_MS;
